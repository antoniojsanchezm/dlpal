...
import { faCheckCircle, faClock, faFileDownload, faList, faListCheck, faPencil, faRepeat, faTimes, faTrash, faVideo, faVolumeHigh, faWrench } from "@fortawesome/free-solid-svg-icons";
    ...
    <Dialog fullScreen open={queue_open} onClose={() => setQueueOpen(false)}>
      <DialogTitle className="bg-[#020617]">
        ...
        <div className="flex items-center gap-4 text-gray-400 text-sm mt-2">
          <div>
            <IconAndText icon={faClock} text={queue.filter((q) => !q.progress || (q.progress?.value && !q.progress?.completed)).length} />
          </div>
          <div>
            <IconAndText icon={faCheckCircle} text={queue.filter((q) => q.progress?.completed).length} />
          </div>
          <div>
            <IconAndText icon={faList} text={queue.length} />
          </div>
        </div>
      </DialogTitle>
          ...
          {(queue.length > 0) ? queue.map((element) => {
            ...
            return (
              <div key={element.data.id}>
                <div className={`flex items-center ${(element.progress?.completed) ? "bg-green-900 hover:bg-green-800" : "bg-gray-600"} text-white p-3 rounded-lg`} style={styles} onClick={async () => {
